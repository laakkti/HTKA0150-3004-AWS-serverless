...
module.exports.handler = async (event) => {
  try {
    ...
    //const getAttr = (name) =>
    //userDetails.UserAttributes.find((attr) => attr.Name === name)?.Value || null;

    //const name = getAttr('name');
    //const lastname = getAttr('family_name');
    ...
    const getAttr = (name) =>
      userResponse.UserAttributes.find((attr) => attr.Name === name)?.Value ||
      null;
