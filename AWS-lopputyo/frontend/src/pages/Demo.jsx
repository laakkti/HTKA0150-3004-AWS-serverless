...
function Demo({ data, geometry, navbarHeight, token }) {
  ...
  const url = "https://vr6mgijfvf.execute-api.eu-north-1.amazonaws.com/ndvi/getimages";

  // HUOM pitäisi hakea data.id:llä!!!! eli rest:iin eri lambda kuin muuten!!!!

  // viimeinen eli ehto parametri vois olla id tarviiko
  const { imageData, loading, error } = useImageRequest(
    url,
    data.id,
    token
  );
  //datesOfYear.length > 0
  //////////////////////////
 ...
 // console.log("data.id=======================",data.id); 
  ...
  const { lastRowHeight, calculatingHeight } = useDynamicRowHeight(
    rowRef,
    initialized
  );
...
//console.log("333333333333333333333333 ",data.dates);

  useEffect(() => {
    if (data && data.dates !== undefined) {
      console.log("44444444444444444444444444444444 ", data.dates);
    
      // ONKO TURHA!!!!
      setDates(data.dates);

      let years = getYears(data.dates);
      console.log("years xxxxxxxxxxxxxxx",years);
      ...
      setDatesOfYear(doy);
      console.log("dates of year xxxxxxxxxxxxxxx",doy);
  ...
  useEffect(() => {
    function func() {
      ...
      console.log("SelectedYear(years[0])",years[0]);
    }
                  ...
                  {datesOfYear.map((item, index) => {
                    return (
                      <option key={index} value={item.sentinelid}>
                        {new Date(item.generationtime).toLocaleDateString('fi-FI')} 
                      </option>
                    );
                  })}
                </Form.Select>
